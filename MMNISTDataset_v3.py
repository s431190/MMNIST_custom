...
class MMNISTDataset(Dataset):
    ...
    def __init__(self, unimodal_datapaths, label_datapaths, transform=None, target_transform=None):
        """
    ...
    def _create_mmnist_dataset(savepath, backgroundimagepath, num_modalities, train):
        ...
        if num_modalities > len(background_filepaths):
            print("debug:",len(background_filepaths))
        ...
        for digit in range(10):
            ...
            for m in range(num_modalities):
                ...
                for i, ix in enumerate(ixs_perm):
                    ...
                    # save as png
                    filepath = os.path.join(savepath, "m%d/%d_%d.png" % (m, i, digit))
    ...
    def __getitem__(self, index):
        ...
        """
        images_dict = {"m0": self.m0[index],
                       "m1": self.m1[index],
                       "m2": self.m2[index],
                       "m3": self.m3[index],
                       "m4": self.m4[index]}
        return images_dict, self.labels[index]  # NOTE: for MMNIST, labels are shared across modalities, so can take one value
...
if __name__ == "__main__":
    ...
    parser.add_argument('--savepath-train', type=str, default="./train")
    parser.add_argument('--savepath-test', type=str, default="./test")
    parser.add_argument('--backgroundimagepath', type=str, default="./backgroundimage")
